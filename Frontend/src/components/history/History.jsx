...
function History() {
  ...
  const handleMonth = () => {
    ...
    let currentMonth = state.user?.expenses.filter(
      (exp) => 
        new Date(exp.date).getMonth() === month && new Date(exp.date).getFullYear()=== year )
    ...
    if (addedCategories[category] && new Date(addedCategories[date]).getMonth===month) {
        ...
        <div className="historyBottom">
          <div className="history-bottom-left">
            ...
            <div className="history-bottom-data">
              {filterType==="month"&&
              <div>
                <button type="submit" onClick={handleLastMoth}>{"<<"}</button>
                <h3>{months[month]}<span>{year}</span></h3>
                <button type="submit" onClick={handleNextMonth}>{">>"}</button>
                </div>}
              <div>
                {monthExp?.map((exp)=>{
               return (
              ...
              <div className="expData total">Total<p>{total}</p></div>
